...
public class PlayerDataStore {
    private final Store<PlayerCosmeticsPersistable, UUID> cosmeticStore;
    ...
    public void saveAll(UUID playerId, PlayerData data) {
        cosmeticStore.save(playerId, data.getCosmetics().toPersistable());
    }

    public PlayerData load(UUID playerId) {
        PlayerData playerData = new PlayerData(playerId);
        var cosmetics = cosmeticStore.findById(playerId);
        cosmetics.ifPresent(persistable -> playerData.getCosmetics().fromPersistable(persistable));
        return playerData;
    }
}
