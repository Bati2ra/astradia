...
public class PlayerLocalCosmeticStore implements Store<JsonObject, UUID> {
    ...
    public PlayerLocalCosmeticStore(MinecraftServer server) {
        this.dataFolder = new File(FabricLoader.getInstance().getGameDir().toFile(), "vento/players/cosmetics");
    ...
    public void save(UUID id, JsonObject entity) {
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            VentoServer.LOGGER.info("{} Successfully saved cosmetics for player {}", LOG_PREFIX, id);
        } catch (IOException e) {
            VentoServer.LOGGER.error("{} Failed to save cosmetics for player {}", LOG_PREFIX, id);
    ...
    public Optional<JsonObject> findById(UUID uuid) {
        ...
        if (!file.exists()) {
            VentoServer.LOGGER.warn("{} No cosmetics file found for player {}. Returning empty object.", LOG_PREFIX, uuid);
            return Optional.of(new JsonObject());
        }

        try (InputStreamReader reader = new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8)) {
            JsonObject data = gson.fromJson(reader, JsonObject.class);
            VentoServer.LOGGER.info("{} Loaded cosmetics for player {}", LOG_PREFIX, uuid);
            return Optional.ofNullable(data);
        } catch (JsonSyntaxException e) {
            VentoServer.LOGGER.error("{} Malformed JSON in cosmetics file for player {}", LOG_PREFIX, uuid);
            VentoServer.LOGGER.debug("{} JsonSyntaxException: {}", LOG_PREFIX, e.getMessage());
            return Optional.of(new JsonObject());
        } catch (IOException e) {
            VentoServer.LOGGER.error("{} Failed to read cosmetics file for player {}", LOG_PREFIX, uuid);
            VentoServer.LOGGER.debug("{} IOException: {}", LOG_PREFIX, e.getMessage());
            return Optional.of(new JsonObject());
        }
    }
    ...
    public void saveAll(Map<UUID, JsonObject> map) {
        ...
        for (Map.Entry<UUID, JsonObject> entry : map.entrySet()) {
            try {
                ...
                VentoServer.LOGGER.warn("{} Failed to save cosmetics for player {}", LOG_PREFIX, entry.getKey());
            }
        }
